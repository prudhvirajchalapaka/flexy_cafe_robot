...
class Task6MultiTableKitchenReturnRobot(Node):
    def __init__(self):
        super().__init__('task6_multi_table_kitchen_return_robot')
        ...
        self.TABLE_TIMEOUT = 15  # 15 seconds for table confirmation
    ...
    def task_loop(self):
        ...
        while rclpy.ok():
            try:
                ...
                # Execute multi-table delivery sequence with kitchen return logic
                self.execute_multi_table_with_kitchen_return(table_numbers)
    ...
    def execute_multi_table_with_kitchen_return(self, table_numbers):
        """Execute the complete multi-table delivery with kitchen return logic"""
        self.get_logger().info(f"\n{'='*60}")
        self.get_logger().info(f"Starting multi-table delivery with kitchen return")
        ...
        # Step 2: Deliver to Multiple Tables with Confirmation
        confirmed_tables = []
        unconfirmed_tables = []
        ...
        for idx, table_number in enumerate(table_numbers, 1):
            ...
            if not self.navigate_to(table_number):
                ...
                unconfirmed_tables.append(table_number)
                continue
            
            self.get_logger().info(f"✓ Reached Table {table_number} successfully!")
            
            # Wait for table confirmation with timeout
            ...
            if table_confirmed:
                ...
                self.get_logger().info(f"🎉 Food successfully delivered to Table {table_number}!")
                confirmed_tables.append(table_number)
            else:
                self.get_logger().warn(f"⚠ No confirmation at Table {table_number}! Skipping to next table...")
                unconfirmed_tables.append(table_number)
            ...
            if remaining > 0:
                self.get_logger().info(f"📊 Progress: {idx}/{len(table_numbers)} visited, {remaining} remaining")
        
        # Step 3: Return to Kitchen (Always, regardless of confirmations)
        self.get_logger().info(f"\nStep 3: Returning to KITCHEN before going home...")
        if unconfirmed_tables:
            self.get_logger().info(f"📋 Returning unconfirmed orders for tables {unconfirmed_tables} to kitchen")
        else:
        ...
        self.get_logger().info(f"   - Tables visited: {len(table_numbers)} {table_numbers}")
        self.get_logger().info(f"   - Confirmed deliveries: {len(confirmed_tables)} tables {confirmed_tables}")
        self.get_logger().info(f"   - Unconfirmed deliveries: {len(unconfirmed_tables)} tables {unconfirmed_tables}")
        self.get_logger().info(f"   - Success rate: {len(confirmed_tables)/len(table_numbers)*100:.1f}%")
...
def main(args=None):
    ...
    robot = Task6MultiTableKitchenReturnRobot()
